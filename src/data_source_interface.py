...
本模块定义了金融数据源的抽象基类和相关异常类，为不同的数据源实现提供统一的接口规范。
所有具体的数据源实现（如BaostockDataSource、AkshareDataSource等）都必须继承并实现这个接口。

主要组件:
- FinancialDataSource: 抽象基类，定义了所有数据源必须实现的方法
- DataSourceError: 数据源异常基类
- LoginError: 登录失败异常
- NoDataFoundError: 数据未找到异常

接口设计原则:
- 统一的方法签名，确保不同数据源的可互换性
- 标准化的返回格式（pandas.DataFrame）
- 完整的异常处理机制
- 灵活的参数配置
